...
from Loadtemporal_data_test import VIPL, MyDataset, Normaliztion, ToTensor
...
def train_test():
    ...
    VIPL_test_list = 'yolov5-test1.txt'
    ...
    model = model#.cuda()
    model.load_state_dict(torch.load(dict_list[0], map_location='cpu'))
    ...
    file_name = 'for_phys_ufc_crop'

    with torch.no_grad():
        for i, sample_batched in enumerate(dataloader_test):
            
            inputs = sample_batched['video_x']#.cuda()
            clip_average_HR, frame_rate = sample_batched['clip_average_HR_peaks'], sample_batched['framerate']#.cuda()
            ...
            for clip in range(inputs.shape[1]):
                ...
                rPPG = rPPG[0, 30:30+160] # 160 프레임으로 잘라서 하는 이유?
                #rPPG = rPPG[0, 30:30+100]
                #print('rppg:', rPPG)
                
                HR_predicted = TorchLossComputer.cross_entropy_power_spectrum_forward_pred(rPPG, frame_rate)+40
                HR += HR_predicted
                print("HR_pred:", HR_predicted.item())
                #HR_predicted = np.max(int(HR_predicted.item()), 100)
                hr_graph.append(HR_predicted.item())
                
                rPPG_long = torch.cat((rPPG_long, rPPG),dim=0)
                plt.plot(rPPG)
                plt.savefig("./"+file_name+"/out/rppg_"+str(clip))
                plt.cla()
    ...
    #plt.show()
    plt.ylim(50, 150)
    plt.savefig("./"+file_name+"/out/hr_graph")
    # visual and save 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_data', type=str, default="./") # /scratch/project_2003204/VIPL_frames_Matlab/
